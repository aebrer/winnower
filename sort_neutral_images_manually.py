...
class Display(object):
    ...
    def _on_key_press(self, event):
        ...
        if event.key == 'left':
            filename = f1.split("/")[2]
            print(filename)
            move(f1, "manual_sort_single/animal/" + filename)
            plt.close(self._fig)

        elif event.key == 'right':
            filename = f1.split("/")[2]
            print(filename)
            move(f1, "manual_sort_single/nan/" + filename)
            plt.close(self._fig)

        elif event.key == 'down':
            filename = f1.split("/")[2]
            print(filename)
            move(f1, "manual_sort_single/group/" + filename)
            plt.close(self._fig)

        elif event.key == 'up':
            filename = f1.split("/")[2]
            print(filename)
            move(f1, "manual_sort_single/obscured/" + filename)
            plt.close(self._fig)
...
photo_dir = "manual_sort_single/neutral/"
